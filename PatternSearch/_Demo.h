...
void PrintArray(std::vector<u_long> array) {
  for (u_long index = 0; index < array.size(); index++) {
    std::cout << array[index] << "\n";
...
char *GenerateRandomString(u_long length, char alphabetType) {
...
void testSearchFull(int tries, string pattern = "ga") {
  char *array;
  vector occurrences;
  u_long length;
  ...
  for (length = 10000; length <= 250000; length += 10000) {
    for (int localTries = 0; localTries <= tries; localTries++) {
      ...
      occurrences = searchBoyerMoore(strarray, pattern);
      ...
      occurrences = searchKMP(array, pattern);
      ...
      occurrences = searchRabinKarp(strarray, pattern, 64);
      ...
      occurrences = searchNaive(strarray, pattern);
      ...
      occurrences = searchBitap(array, pattern);
