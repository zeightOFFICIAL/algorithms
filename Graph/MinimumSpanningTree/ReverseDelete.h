...
void GraphTraverse(int start_vertex, std::vector<bool> &visited,
                   std::vector<std::vector<int>> graph);
std::vector<std::vector<int>>
MSTReverseDelete(std::vector<std::vector<int>> edges_list,
                 int number_of_vertices);
...
bool GraphConnected(std::vector<std::vector<int>> edges_list,
  ...
  for (int each_edge = 0; each_edge < edges_list.size(); each_edge++) {
    graph[edges_list[each_edge][1]][edges_list[each_edge][2]] =
        static_cast<bool>(edges_list[each_edge][0]);
    graph[edges_list[each_edge][2]][edges_list[each_edge][1]] =
        static_cast<bool>(edges_list[each_edge][0]);
  }
...
MSTReverseDelete(std::vector<std::vector<int>> edges_list,
  ...
  for (int each_vertex = edges.size() - 1; each_vertex >= 0; each_vertex--) {
    ...
    if (!GraphConnected(edges, number_of_vertices)) {
      ...
      mst_redelete_table.push_back({edges[each_vertex][1],
                                    edges[each_vertex][2],
                                    edges[each_vertex][0]});
    }
  }
