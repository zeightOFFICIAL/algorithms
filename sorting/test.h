...
*/
long long *GenerateRandomArray(unsigned long amount_of_elements,
                               long long max_value, long long min_value) {
...
*/
long long *GenerateArrayFile(std::string filename,
...
*/
template <typename T>
void WriteProof(std::string filename, T *array, T *array2, T *array3,
                unsigned long long amount_of_elements) {
  std::ofstream file;
  ...
  if (!file)
    ...
    file
        << "The only proof of the correctness of all\nthese sorting algorithms "
           "is actual exactness\nof how they've performed. The equality "
...
*/
template <typename T>
void WriteArray(std::string filename, T *array,
                unsigned long amount_of_elements) {
  std::ofstream file;
  file.open(filename, std::ios::trunc);
  if (!file)
...
void fulltest(int tries) {
  ...
  std::chrono::steady_clock::time_point start, end, start2, end2, start3, end3,
      start4, end4, start5, end5; // time stamps
  int local_length;
