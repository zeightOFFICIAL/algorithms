...
long long* GenerateRandomArray(u_long length, d_long max_value, d_long min_value) {
	d_long* array = new d_long[EXTREME_LENGTH];
	srand((int)time(0));
...
{
	d_long* array, * array2, * array3, * array5;
	d_double alltime = 0, alltime2 = 0, alltime3 = 0, alltime5 = 0; //time summers    
	int min_value = -15000, max_value = 15000; //intervals of generating numbers
	int local_length;
	std::chrono::steady_clock::time_point start, end, start2, end2, start3, end3, start5, end5; //time stamps
	for (local_length = 5000; local_length <= 100000; local_length += 5000) {
		std::cout << local_length << "\n";
		alltime = 0, alltime2 = 0, alltime3 = 0, alltime5 = 0;
...
{
	d_long* array4;
	d_double alltime4 = 0; //time summers
	int local_length;
	int min_value = -15000, max_value = 15000; //intervals of generating numbers
	std::chrono::steady_clock::time_point start4, end4; //time stamps
	for (local_length = 5000; local_length <= 100000; local_length += 5000) {
		std::cout << local_length << "\n";
		alltime4 = 0;
		for (int local_tries = 0; local_tries < tries; local_tries++)
