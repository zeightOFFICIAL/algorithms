...
std::vector<unsigned long> RabinKarp(std::string text, std::string pattern, int alphabet_power = 256);
...
/*
        ...
        in text.
*/
std::vector<unsigned long> RabinKarp(std::string text, std::string pattern, int alphabet_power) {
  unsigned long text_length = text.length(), pattern_length = pattern.length(),
         through_pattern;
  ...
  for (unsigned long letter_in_pattern = 0; letter_in_pattern < pattern_length - 1;
       letter_in_pattern++)
  ...
  for (unsigned long through_text = 0; through_text <= text_length - pattern_length;
       through_text++) {
    if (pattern_hash == text_hash) {
      for (through_pattern = 0; through_pattern < pattern_length;
           through_pattern++) {
    ...
    if (through_text < text_length - pattern_length) {
      text_hash = (d_capacity * (text_hash - text[through_text] * h_value) +
                   text[through_text + pattern_length]) %
                  q_prime;
      if (text_hash < 0)
