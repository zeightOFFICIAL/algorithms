...
std::vector<unsigned long> BoyerMoore(std::string text, std::string pattern) {
  ...
  long long text_index = 0, pattern_index;
  ...
  while (text_index <= (text_length - pattern_length)) {
    ...
    while (pattern_index >= 0 &&
           pattern[pattern_index] == text[text_index + pattern_index])
      --pattern_index;
    if (pattern_index < 0) {
      occurance_points.push_back(text_index);
      text_index +=
          (text_index + pattern_length < text_length)
              ? pattern_length -
                    heuristics_table[text[text_index + pattern_length]]
              : 1;
    } else
      text_index +=
          MaxByValue(1, pattern_index -
                            heuristics_table[text[text_index + pattern_index]]);
  }
...
void BadCharHeuristics(std::string pattern, unsigned long pattern_length,
                       int *heuristics_table, unsigned long text_length) {
  for (unsigned long letter_from_alphabet = 0; letter_from_alphabet < 256;
       letter_from_alphabet++)
    heuristics_table[letter_from_alphabet] = -1;
  for (unsigned long letter_from_pattern = 0;
       letter_from_pattern < pattern_length; letter_from_pattern++)
