...
// -------------------------------------------------------------------------------------------------------------------------
void TestSorting(void (*sorting)(long long *, unsigned long, bool), unsigned int length, bool order = true, int maxValue = 15000, int minValue = -15000) {
...
void TestAllSortings(int tries, unsigned int maxSize, unsigned int step, int maxValue = 15000, int minValue = -15000) {
  ...
  std::chrono::steady_clock::time_point start1, end1, start2, end2, start3,
      end3, start4, end4, start5, end5, start6, end6, start7, end7, start8,
      end8, start9, end9;
  ...
  for (localLength = 0; localLength <= maxSize; localLength += step) {
    ...
    allequal =
        CheckCorrect(arr1, localLength) && CheckCorrect(arr2, localLength) &&
...
void TestSillySortings(int tries) {
  ...
  for (localLength = 0; localLength <= 300; localLength += 50) {
    ...
    for (int localTries = 0; localTries < tries; localTries++) {
      ...
      arr1 = GenerateRandomArray(localLength, max_value, min_value);
      start1 = std::chrono::steady_clock::now();
      sorting::SlowSort(arr1, localLength);
      end1 = std::chrono::steady_clock::now();
      alltime1 = alltime1 + std::chrono::duration_cast<std::chrono::microseconds>(end1 - start1).count() /1000000.0;
    }
    std::cout << "Stooge sort: Average time: " << std::setprecision(9)
              << (alltime / tries) << " seconds"<< "\n";
    std::cout << "Slow sort: Average time: " << std::setprecision(9)
              << (alltime1 / tries) << " seconds"<< "\n";
    std::cout << "Total elements: " << localLength << "\n";
    std::cout << "Total tries: " << tries << "\n";
    std::cout << "Is correct: " << (CheckCorrect(arr, localLength) && CheckCorrect(arr1, localLength)) << "\n";
    std::cout << "\n";
  }
  delete[] arr;
}